...
class Chromosome:
    def __init__(self, input_list: List[Route]) -> None:
        if (
            ...
            or len(input_list) < 1
            or not isinstance(input_list[0], Route)
        ):
            raise TypeError
        else:
    ...
    def BCRC_crossover(
        self, original_path: List[Route], node_list: List[Node]
    ):
                ...
                if route.storage_used + n.demand <= CAR_CAPACITY:
                    for i in range(route.length + 1):
                        ...
                        if (
                            ...
                            < best_route_distance_improvement
                        ):
                            best_route_distance_improvement = (
                                new_route.route_distance - route.route_distance
                            )
    ...
    def __str__(self):
        general_info = [
            "Number of routes: {}\nDistance: {}".format(
                len(self.path), self.distance
            )
        ]
        path_info = [
            "Route {} -> {} (Distance: {})".format(
                i + 1, r.route, r.route_distance
            )
...
def generate_population(n_population: int = 10):
    return sorted(
        [Chromosome.from_node_list(customer_list) for i in range(n_population)]
    )
